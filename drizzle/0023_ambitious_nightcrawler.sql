CREATE TABLE `ares_configurations` (
	...
	CONSTRAINT `ares_configurations_id` PRIMARY KEY(`id`)
);
...
CREATE TABLE `ares_validation_history` (
	...
	CONSTRAINT `ares_validation_history_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
CREATE TABLE `ares_validation_rules` (
	...
	`createdAt` timestamp NOT NULL DEFAULT (now()),
	`updatedAt` timestamp NOT NULL DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP,
	CONSTRAINT `ares_validation_rules_id` PRIMARY KEY(`id`)
);
