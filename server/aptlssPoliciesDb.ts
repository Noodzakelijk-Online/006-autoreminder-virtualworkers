...
export async function getAllPolicies(): Promise<AptlssOperationalPolicy[]> {
  ...
  if (!db) return [];
  const rows = await db.select().from(aptlssOperationalPolicies).orderBy(aptlssOperationalPolicies.category, aptlssOperationalPolicies.ruleKey);
  if (rows.length === 0) {
    for (const policy of DEFAULT_POLICIES) {
      await db.insert(aptlssOperationalPolicies).values(policy as AptlssOperationalPolicy).onDuplicateKeyUpdate({ set: { label: policy.label } });
    }
...
export async function upsertWeeklyAnalysis(snapshot: Omit<WeeklyAnalysisSnapshot, "id" | "createdAt" | "generatedAt">): Promise<void> {
  ...
  if (existing) {
    await db.update(weeklyAnalysisSnapshots).set({ ...snapshot }).where(eq(weeklyAnalysisSnapshots.weekKey, snapshot.weekKey));
  } else {
...
export async function getAutopilotLevel(): Promise<number> {
  const val = await getPolicyValue("autopilot_level", "2");
