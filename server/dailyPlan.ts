...
import type { DailyPlanGenerationPhase } from "./dailyPlanGenerationProgress";
...
type DailyPlanProgressReporter = (
...
export async function generateDailyPlan(
  dateKey = eatDateKey(),
  generatedBy: DailyPlanPayload["generatedBy"] = "manual",
  reportProgress?: DailyPlanProgressReporter,
) {
  await assertDailyPlanDbAvailable();
  reportProgress?.("constraints");
  ...
  if (!constraints.isWorkday) {
    ...
    reportProgress?.("persisting", "Saving the protected off-day plan.");
    await saveDailyPlan(payload, generatedBy === "auto");
    return payload;
  }

  reportProgress?.("work_context");
  let { summaries, robertItems } = await buildSummaries();
  let usedLiveTrelloFallback = false;
  if (summaries.length === 0) {
    reportProgress?.("work_context", "No saved card plans were found; reading live Trello cards.");
  ...
  reportProgress?.(
    "ai_planning",
    aiPlannerConfigured
      ? "Sequencing the strongest next actions and reviewing the schedule with the configured model."
      : "Sequencing the strongest next actions with deterministic planning.",
  );
  try {
