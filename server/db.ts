...
/**
 * Increment the demeritD1 counter on the weekly pay log for the week containing `dateEAT`.
 * If no pay log exists for that week yet, creates one with only demeritD1 set.
 * Returns the updated/created row id.
 */
export async function incrementPayLogD1(dateEAT: string, d1Count: number): Promise<{ id: number | null; projectedPay: number }> {
  ...
  const existing = await getWeeklyPayLogByWeek(weekStart);
  ...
  // Use same dollar amounts per unit as upsertWeeklyPayLog / frontend
  ...
  if (existing) {
    // Rebuild all counts from existing row and bump D1
    const result = await upsertWeeklyPayLog({
      weekStart,
      weekEnd,
      paymentCycleId: existing.paymentCycleId ?? undefined,
      meritM1: Number(existing.meritM1),
      meritM2: Number(existing.meritM2),
      meritM3: Number(existing.meritM3),
      meritStreak: Number(existing.meritStreak),
      demeritD1: newD1,
      demeritD2: Number(existing.demeritD2),
      demeritD3: Number(existing.demeritD3),
      demeritD4: Number(existing.demeritD4),
      demeritD5: Number(existing.demeritD5),
      demeritD6: Number(existing.demeritD6),
      demeritD7: Number(existing.demeritD7),
      demeritD8: Number(existing.demeritD8),
      demeritD9: Number(existing.demeritD9),
      demeritD10: Number(existing.demeritD10),
      demeritD11: Number(existing.demeritD11),
      notes: existing.notes ?? undefined,
    });
    return { id: (result as any).id ?? existing.id, projectedPay };
  } else {
    // Create a new pay log row with only D1 populated
    const result = await upsertWeeklyPayLog({
      weekStart,
      weekEnd,
      meritM1: 0, meritM2: 0, meritM3: 0, meritStreak: 0,
      demeritD1: newD1,
      demeritD2: 0, demeritD3: 0, demeritD4: 0, demeritD5: 0,
      demeritD6: 0, demeritD7: 0, demeritD8: 0, demeritD9: 0,
      demeritD10: 0, demeritD11: 0,
    });
    return { id: (result as any).id ?? null, projectedPay };
  }
}

// ─── Daily Schedule Settings helpers ─────────────────────────────────────────
